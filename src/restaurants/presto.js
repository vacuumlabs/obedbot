...
const MENU_LINK = 'https://www.bizref.sk/sk/firmy/charopos-s-r-o/denne-menu/837/OBEDOVE-MENU/'
...
const help =
`Napíšete pred ${toHumanTime(endOfOrders)} do Slacku presto+"číslo"+"p"+(1/2), kde 1 alebo 2 na konci je číslo polievky.
...
function getOrdersCounter() {
  return new OrdersCounter(id, name, ORDER_PATTERN, {
    getGroups: ({ groups: { typePresto, typePizza, main, soup, pizza, size } }) => {
      if (typePresto) {
        return { main, soup }
      }
      ...
      return { pizza: pizzaSize === DEFAULT_PIZZA_SIZE ? pizzaNum : `${pizzaNum} veľkosti ${pizzaSize}` }
    },
    viewData: {
      totalMeals: 7,
    },
